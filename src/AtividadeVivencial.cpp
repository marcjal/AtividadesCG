...
int main()
{
    ...
    GLFWwindow *window = glfwCreateWindow(
        WIDTH,
        HEIGHT,
        "Atividade Vivencial - OBJ, Selecao e Transformacoes",
        nullptr,
        nullptr);
...
void applyTransformation(int key, int mods)
{
    ...
    else if (currentMode == TransformMode::ROTATE)
    {
        ...
        // Use Shift + eixo para rotacionar no sentido contrário.
        if (key == GLFW_KEY_X)
        {
...
int parseOBJVertexIndex(const string &token, int vertexCount)
{
    ...
    // OBJ normalmente usa índice começando em 1.
    if (index > 0)
    {
        return index - 1;
    }

    // Suporte opcional para índices negativos do OBJ.
    // -1 significa último vértice.
    if (index < 0)
    {
